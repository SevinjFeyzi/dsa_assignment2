...
undo_dropdown = Dropdown(900, 200, 200, 50, ['Undo', 'Undo'])
p_dropdown = Dropdown(900, 270, 200, 50, ['[+]','[+]'])
level_dropdown = Dropdown(900, 340, 200, 50, ['AI Level: 3'])
n_dropdown = Dropdown(900, 400, 200, 50, ['[-]','[-]'])
...
while running:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            undo_dropdown.handle_event(event)
            n_dropdown.handle_event(event)
            p_dropdown.handle_event(event)
            
            choice[0] = player1_dropdown.get_choice()
            choice[1] = player2_dropdown.get_choice()
            choice[2] = undo_dropdown.get_choice()
            # level increase 
            if p_dropdown.get_choice():
                LEVEL+=1
                level_dropdown.options = [f'AI Level: {LEVEL}']
                p_dropdown.current_option = 0
            if n_dropdown.get_choice():
                LEVEL-=1
                if LEVEL < 0:
                    LEVEL = 0
                level_dropdown.options = [f'AI Level: {LEVEL}']
                n_dropdown.current_option = 0
            # undo for AI
            if choice[2] and (choice[0] or choice[1]):
                board.turn -=2
                if board.turn < 0:
                    board.turn = 0
                if board.turn in old_board:
                    board.set(old_board[board.turn][1])
                    current_player = old_board[board.turn][0]
                undo_dropdown.current_option = 0
            # undo
            if choice[2]:
                board.turn -=1
                if board.turn < 0:
    ...
    if not has_winner:
        if overflowing:
            ...
            if choice[current_player] == 1:
                bots = [PlayerOne(level=LEVEL),PlayerTwo(level=LEVEL)] # level set
            ...
            if make_move:
                # old_board = board
                old_board[board.turn] = [current_player , board.get_board()]
    ...
    n_dropdown.draw(window)
    p_dropdown.draw(window)
